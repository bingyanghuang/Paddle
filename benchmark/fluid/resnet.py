...
def resnet_imagenet(input, class_dim, depth=50, data_format='NCHW'):
    ...
    pool2 = fluid.layers.pool2d(
        ...
        pool_stride=1,
        global_pooling=True)
    out = fluid.layers.fc(input=pool2, size=class_dim, act='softmax', bias_attr=False)
    return out
...
def run_benchmark(model, args):
    ...
    if args.use_fake_data:
        train_reader = paddle.batch(
            user_data(fake_data), batch_size=args.batch_size)
    else:
        train_reader = paddle.batch(
    ...
    for pass_id in range(args.pass_num):
        ...
        for batch_id, data in enumerate(train_reader()):
            ...
            if not args.use_fake_data:
                ...
                label = np.array(map(lambda x: x[1], data)).astype('int64')
                label = label.reshape([-1, 1])
            loss, acc, weight, pred = exe.run(
                fluid.default_main_program(),
                feed={'data': image,
                      'label': label},
                fetch_list=[avg_cost, batch_acc, batch_size_tensor,predict])
            iters += 1
            num_samples += len(label)
            accuracy.add(value=acc, weight=weight)
            train_losses.append(loss)
            train_accs.append(acc)
            print("Pass: %d, Iter: %d, Loss: %f, Accuracy: %f, FPS: %.5f img/s" %
                  (pass_id, iters, loss, acc, num_samples / (time.time() - start_time)))
	 ...
	 # Save model 
        if args.save_models: 
            save_models_path = "{}/pass{}{}/".format(args.save_models_path, 
                                                      pass_id, appendix) 
            if not os.path.isdir(save_models_path): 
                os.makedirs(save_models_path) 
            fluid.io.save_inference_model(save_models_path, 
